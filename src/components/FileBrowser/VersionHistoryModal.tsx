...
import { getFileVersions, restoreFileVersion } from "@/app/(dashboard)/folders/actions"
        ...
        <div className="p-6 max-h-[60vh] overflow-y-auto bg-slate-50/50">
          {isLoading ? (
            ...
            <div className="space-y-3">
              {versions.map((v, i) => (
                <div key={v.id} className="flex items-center justify-between bg-white p-4 rounded-2xl shadow-sm ring-1 ring-slate-100">
                  <div className="flex flex-col">
                    <span className="font-bold text-slate-800 text-sm">
                      Versi {versions.length - i}
                    </span>
