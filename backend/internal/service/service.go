...
// UserByEmail mengambil data profil user dari kemenag_arsip.users.
func (s *Services) UserByEmail(ctx context.Context, email string) (*domain.User, error) {
	return s.repos.User.GetUserByEmail(ctx, email)
}
